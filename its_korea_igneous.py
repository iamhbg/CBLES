...
def resolve_api_key(sidebar_key):
    """
    API 키를 여러 경로에서 안전하게 찾는다.
    우선순위: 사이드바 입력 > st.secrets > 환경변수(GEMINI_API_KEY / GOOGLE_API_KEY)
    """
    if sidebar_key:
...
def render_sidebar():
    """
    사이드바: API 키 입력, 모델 선택, 대화 초기화, 학습 안내.
    Returns: (sidebar_api_key, model_name)
    """
    with st.sidebar:
        st.header("⚙️ 설정")
        ...
        sidebar_api_key = st.text_input(
            ...
            help="https://aistudio.google.com/app/apikey 에서 발급. "
                 "또는 환경변수 GEMINI_API_KEY / .streamlit/secrets.toml 로도 인식됩니다.",
            placeholder="AI... 형식의 키를 붙여넣으세요",
        )
    ...
    return sidebar_api_key, model_name
...
def render_chat_panel(results, api_key, model_name):
    ...
    if not GENAI_AVAILABLE:
        ...
        st.warning(
            "왼쪽 사이드바에 **Gemini API 키**를 입력하면 AI 튜터와 대화할 수 있습니다."
        )
...
def main():
    ...
    # 사이드바 + 상단 헤더
    sidebar_api_key, model_name = render_sidebar()
    ...
    # --- 우측: AI 튜터 챗봇 ---------------------------------------------------
    with col_right:
        chat_ready = render_chat_panel(results, resolve_api_key(sidebar_api_key), model_name)

    # 실제 사용할 API 키 (사이드바/secrets/환경변수 통합)
    effective_api_key = resolve_api_key(sidebar_api_key)

    # --- 하단: 채팅 입력창 (앱 루트에 배치하여 화면 하단에 고정) --------------
    user_input = st.chat_input(
        "여기에 네 생각을 입력해 보자…"
        if chat_ready
        else "먼저 API 키를 입력하고 라이브러리를 설치하세요.",
