...
2. 시공간적(냉각·분포) 인과축 ─ '어디서·언제 식었나':
   ...
   · 중생대 지각 변동 → 지하 심부 서랭 → 조립질 심성암(화강암) → 설악산·북한산·월출산 (이후 융기·풍화·침식으로 지표에 노출)
    ...
    data = {
        "현무암": {
            ...
            "photos": [
                ...
                {
                    "path": "images/basalt_hantangang.jpg",
                    "caption": "한탄강 재인폭포 — 현무암 용암이 굳어 만든 협곡과 주상절리 폭포 (유네스코 세계지질공원)",
        ...
        "화강암": {
            ...
            "process": "중생대의 지각 변동으로 생성된 유문암질 마그마가 지하 깊은 곳에서 "
                       ...
                       "풍화·침식으로 제거되면서 지표에 드러났습니다.",
            "structure": "판상 절리 ─ 지하 깊은 곳의 암석이 융기하면서 압력이 감소하여 "
                         "만들어진 얇은 판 모양의 절리. 지하에서 천천히 굳었다는 증거.",
            "emphasized": True,
            "photos": [
                {
                    "path": "images/granite_insubong.jpg",
                    "caption": "북한산 인수봉 — 중생대에 지하에서 굳은 화강암이 융기·풍화·침식으로 드러난 절벽",
            ...
            "landmarks": [
                "**북한산 인수봉(서울·경기)** — 중생대에 지하에서 굳은 화강암이 융기·풍화·침식으로 "
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if st.button("🗑️ 대화 초기화", use_container_width=True,
                     help="대화 기록과 학습 진행 상태를 모두 초기화합니다."):
        ...
        with st.expander("📖 사용설명서", expanded=False):
            ...
            st.info(
                "한반도의 화성암(현무암·화강암 등)이 "
                "**언제·어디서·어떻게** 만들어졌는지를 "
                "직접 탐구하는 **지능형 학습 도구**입니다.\n\n"
                "- **왼쪽** — 마그마 조건을 바꾸는 시뮬레이터\n"
                "- **오른쪽** — 여러분의 생각을 이끌어 주는 AI 튜터"
            )

            st.divider()

            # 2. 시뮬레이터 사용법
            st.markdown("#### 🎛️ 시뮬레이터 사용법")
            st.markdown(
                "**① SiO₂ 함량 슬라이더 (45 ~ 75 %)**\n\n"
                "마그마 속 이산화규소 비율입니다.\n"
                "- **낮을수록** → 현무암질 마그마 → 어두운 암석\n"
                "- **높을수록** → 유문암질 마그마 → 밝은 암석\n\n"
                "**② 냉각(관입) 깊이 슬라이더 (0 ~ 12 km)**\n\n"
                "마그마가 식은 위치입니다.\n"
                "- **0 km** → 지표 분출 · 급랭 → 세립질 화산암\n"
                "- **깊을수록** → 지하 서랭 → 조립질 심성암\n\n"
                "두 슬라이더를 바꾸면 결정 알갱이 모식도·암석 이름·"
                "한반도 산출지가 실시간으로 바뀝니다."
            )
            ...
            # 3. AI 튜터 사용법
            st.markdown("#### 🤖 AI 튜터 사용법")
            st.markdown(
                "화면 **오른쪽 아래 입력창**에 자유롭게 생각을 적어 보세요.\n\n"
                "**이런 식으로 시작해도 좋아요:**\n"
                "- `제주도는 왜 현무암이에요?`\n"
                "- `슬라이더를 바꿔봤더니 화강암이 됐는데, 왜 이렇게 되는 건가요?`\n"
                "- `모르겠어요 — 힌트 주세요`\n\n"
                "> 💡 AI 튜터는 정답을 먼저 알려주지 않습니다.\n"
                "> 여러분의 대답에 되물으며 스스로 원리를 발견하도록 이끌어 줍니다.\n"
                "> 전문 용어를 몰라도, 일상 언어로 설명해도 됩니다."
            )
            st.warning(
                "**학습 진행에 따라 절리 모식도가 자동으로 나타납니다.**\n\n"
                "AI 튜터가 `위에 나온 그림을 보면…` 이라고 할 때 그림을 활용해 보세요."
            )

            st.divider()

            # 4. 학습 목표
            st.markdown("#### 🎯 이걸 설명할 수 있으면 성공!")
            st.success(
                "**두 가지 인과 관계를 자신의 언어로 연결해 보세요.**\n\n"
                "**① 조성 축**\n"
                "판 경계 환경 → 마그마 조성 → 암석 종류\n\n"
                "**② 냉각·분포 축**\n"
                "냉각 위치 → 조직(세립/조립) → 한반도의 시대별 분포\n\n"
                "**예시 설명:**\n"
                "신생대 한반도 아래에서 판이 섭입하며 현무암질 마그마가 만들어졌고, "
                "지표에서 급랭해 제주도·한탄강의 현무암이 되었다."
            )
